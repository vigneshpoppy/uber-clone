...
export const UberProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const response = await fetch('/api/map1/getDuration', {
          method: 'POST',
          headers: {
  ...
  const createLocationCoordinatePromise = (locationName, locationType) => {
    return new Promise(async (resolve, reject) => {
      try {
        const response = await fetch('api/map1/getLocationCoordinates', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const requestToCreateUserOnSanity = async address => {
    if (!window.ethereum) return
    try {
      await fetch('/api/DataProvider/createUser', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const requestToGetCurrentUsersInfo = async walletAddress => {
    try {
      const response = await fetch(
        `/api/DataProvider/getUserInfo?walletAddress=${walletAddress}`,
      )
